<?php 
    ...
    //EJECUACION DE MODALS DE LA VISTA 
    ...
    {
        if(ENVIRONMENT == 1){
            $asunto = $data['asunto'];//ESTAMOS RECIBIENDO LOS DATOS DEL ARRAY DEL METODO resetPass() DEL CONTROLADOR Login
            $emailDestino = $data['email'];
            $empresa = NOMBRE_REMITENTE;//RECIBIMOS LA VARIABLES GLOBALES DE LA CONFIGURACION
            $remitente = EMAIL_REMITENTE;
            $emailCopia = !empty($data['emailCopia']) ? $data['emailCopia'] : "";
            //ENVIO DE CORREO 
            //ENCABESADOS PARA QUE EL CORREO SEA ENVIADO Y NO CAIGA EN SPAM
            $de = "MIME-Version: 1.0\r\n";//VERSION DEL CORREO
            $de .= "Content-type: text/html; charset=UTF-8\r\n";//ESPECIFICA EL TIPO DE CODIFICACION
            $de .= "From: {$empresa} <{$remitente}>\r\n";//ESPECIFICA EL REMITENTE DE DONDE LO ESTOY ENVIANDO
            $de .= "Bcc: $emailCopia\r\n";
            ob_start();
            require_once("Views/Template/Email/".$template.".php");//REQUIERE EL ARCHIVO DE LA VISTA DEL EMAIL QUE LO ESTAMOS ASIGNANDO EN EL CONTROLADOR Login VARIABLE $sendEmail DEL METODO resetPass()
            $mensaje = ob_get_clean();
            $send = mail($emailDestino, $asunto, $mensaje, $de);//MAIL() CORRESPONDE A LA FUNCION PHP MAIL() QUE SE ENCARGA DE ENVIAR EL CORREO
            return $send;
        }else{
           //Create an instance; passing `true` enables exceptions
            ...
            try {
                ...
                $mail->Password   = 'xxbiebrrsudgffiu';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('domosyensambles01@gmail.com', 'Servidor Local');
