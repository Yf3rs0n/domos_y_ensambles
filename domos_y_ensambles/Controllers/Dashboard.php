<?php 
	...
	class Dashboard extends Controllers{
		public function __construct()
		{
			...
			if(empty($_SESSION['login']))//SI NO EXISTE LA SESION
			{
				header('Location: '.base_url().'/login');//REDIRECCIONAMOS AL LOGIN
				die();//CIERRA LA EJECUCION DE LA PAGINA
			}
		...
		{
			$data['page_id'] = 2;
			$data['page_tag'] = "Dashboard - Domos y Ensambles";
			$data['page_title'] = "Dashboard - Domos y Ensambles";
			$data['page_name'] = "dashboard";
			$data['page_functions_js'] = "functions_dashboard.js";
			$data['usuarios'] = $this->model->cantUsuarios();
			$data['clientes'] = $this->model->cantClientes();
			$data['servicios'] = $this->model->cantServicios();
			$data['contratos'] = $this->model->cantContratos();
			$data['contratos'] = $this->model->cantContratos();
			$data['lastOrders'] = $this->model->lastOrders();
			$data['serviciosTen'] = $this->model->serviciosTen();
